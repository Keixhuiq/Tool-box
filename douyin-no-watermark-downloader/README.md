...
| `content.js` | 页面侧主逻辑，负责悬浮按钮、Toast、当前作品识别和下载调度。 |
| `background.js` | Service worker，维护 Referer 重写规则，并解析抖音 CDN 重定向。 |
| `popup.html` / `popup.css` / `popup.js` | 扩展弹窗和设置面板。 |
| `style.css` | 注入到抖音页面的悬浮按钮样式。 |
...
打开任意抖音视频/图文页面（包括喜欢/收藏列表里打开的弹窗作品），按 `Shift + D` 或点右下角的红色按钮即可下载。

按右上角扩展图标可以打开设置面板，调整画质、文件名模板、保存目录、按钮显隐等。
